...
function AIRecommendationsTab() {
  const { recommendations, applyAIRecommendation, resetRecommendations } = useSimulator();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="md" onClick={resetRecommendations} className="gap-2 font-bold text-slate-700">
      ...
      {recommendations.length === 0 ? (
        <div className="bg-white rounded-3xl border border-emerald-200 p-10 text-center space-y-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700 mx-auto">
            <ShieldCheck className="h-6 w-6" />
          </div>
          <p className="text-lg font-bold text-slate-900">All Available AI Strategies Have Been Applied!</p>
          <p className="text-sm text-slate-500 max-w-md mx-auto">
            Your recurring collections are operating with zero-penalty guardrails. Click below to re-evaluate telemetry for new opportunities.
          </p>
          <Button size="md" onClick={resetRecommendations} className="gap-2 font-bold">
            <RefreshCw className="h-4 w-4" />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 lg:gap-8">
          {recommendations.map((ins) => {
            const isApplied = appliedMap[ins.id];
            return (
              <div
                key={ins.id}
                className="bg-white rounded-3xl border border-slate-200 p-7 xl:p-8 flex flex-col justify-between gap-6 card-hover shadow-xs transition-all"
              >
                <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-xs font-extrabold uppercase tracking-wider text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded-md">
                          {ins.category ? ins.category.replace('_', ' ') : 'STRATEGY'}
                        </span>
                        <p className="text-xs text-slate-400 mt-1 font-mono">
                          Confidence: {ins.confidenceScore || 94}%
                        </p>
                      </div>
                    </div>
                    <span className="text-sm font-extrabold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">
                      {ins.impactMetric || `+₹${ins.expectedGain?.toLocaleString('en-IN')}`}
                    </span>
                  </div>
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <Button
                    ...
                    disabled={isApplied}
                    className={isApplied ? 'bg-emerald-600 hover:bg-emerald-600 gap-1.5 font-bold' : 'gap-1.5 font-bold'}
                  >
                    <Zap className="h-4 w-4" />
                    {isApplied ? 'Applied!' : (ins.recommendedAction || 'Apply Strategy')}
                  </Button>
                </div>
              </div>
            );
